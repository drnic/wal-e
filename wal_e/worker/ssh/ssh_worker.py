...
class TarPartitionLister(object):
    def __init__(self, ssh_conn, layout, backup_info):
    ...
    def __iter__(self):
        ...
        for file_ref in self.ssh_conn.list_files(prefix):
...
class BackupFetcher(object):
    def __init__(self, ssh_conn, layout, backup_info, local_root, decrypt):
        self.ssh_conn = ssh_conn
        self.layout = layout
    ...
    def fetch_partition(self, partition_name):
        ...
        uri = 'ssh://{host}/{path}'.format(host=self.layout.store_name(),
                                              path=part_abs_path)
        with get_download_pipeline(PIPE, PIPE, self.decrypt) as pl:
            # ssh.write_and_return_error(uri, self.ssh_conn, pl.stdin)
            g = gevent.spawn(ssh.write_and_return_error,
                             self.ssh_conn,
                             uri,
                             pl.stdin)
...
class DeleteFromContext(_DeleteFromContext):
    ...
    def __init__(self, ssh_conn, layout, dry_run):
        super(DeleteFromContext, self).__init__(ssh_conn, layout, dry_run)
